...
export default function CreatePost() {
  ...
  async function setPoster(){
    ...
    if(tempRecords.length !== -1){
      const response = await fetch(
        `//52.15.93.98:5050/users/${global.USERID}`
      );
      const userWithPost = {
        name: response.name,
    ...
    await fetch(`//52.15.93.98:5050/users/${global.USERID}`, {
      method: "PATCH",
  ...
  async function getUser() {
    const response = await fetch(`//52.15.93.98:5050/users/${ID}`);
  ...
  async function onSubmit(e) {
    ...
    await fetch("//52.15.93.98:5050/posts", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (tempRecords.length !== -1) {
      // This will send a post request to update the data in the database.
      await fetch(`//52.15.93.98:5050/users/${ID}`, {
        method: "PATCH",
        body: JSON.stringify(userwithpost),
        headers: {
          "Content-Type": "application/json",
